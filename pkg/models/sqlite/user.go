...
func (m *UserModel) Latest() ([]*models.Post, error) {
	stmt := `SELECT id, user_id, title, content, tags, created, username, like, dislike FROM posts`
	...
	for rows.Next() {
		...
		err = rows.Scan(&s.Id, &s.AuthorId, &s.Title, &s.Description, &s.Tags, &s.Created, &s.UserName, &s.Like, &s.Dislike)
		if err != nil {
			return nil, err
		}
...
func (m *UserModel) GetPostsByTag(tag string) ([]*models.Post, error) {
	stmt := `SELECT id, title, content, tags, created, username, like, dislike
	...
	for rows.Next() {
		p := &models.Post{}
		err := rows.Scan(&p.Id, &p.Title, &p.Description, &p.Tags, &p.Created, &p.UserName, &p.Like, &p.Dislike)
		if err != nil {
			return nil, err
		}
